...
def readCamerasFromCustomBlenderTransforms(path, transformsfile, white_background, extension=".png"):
    cam_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, cam_data in enumerate(cam_datas):
            # if is_test==False:
            cam_name = os.path.join(path, "./train/image" + str(idx)+ "0022" + extension)
            # else:
...
def readCustomBlenderDataInfo(path, white_background, eval, extension=".png"):

    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromCustomBlenderTransforms(path, "transforms_train.json", white_background, extension)
    # print("Reading Test Transforms")
